...
The fork adds seven general public controls that Kora requires:
...
7. `Memory.getAll(...).collection` provides stable `updated_at_desc` paging,
   optional text containment and expired-only selection, plus `total` and
   `complete` collection metadata. The in-memory vector store's collection
   listing reads only IDs and payloads instead of decoding unused vectors.
...
the internal erase operation consumed by the public `forget` method. Collection
paging is opt-in; ordinary `getAll`, extraction, embedding, search, ordinary
single-record delete/history, expiration, provider, and scoring behavior are
otherwise unchanged.
